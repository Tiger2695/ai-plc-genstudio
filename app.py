...
# =============================================================================
st.set_page_config(
    page_title="AI-PLC Delta AS228 Studio", page_icon="⚡", layout="wide"
)
...
<style>
    ...
    section[data-testid="stSidebar"] {
        /* Let Streamlit theme handle sidebar background naturally */
        /* This ensures Light theme shows light background, Dark theme shows dark */
    }
    
    /* Text elements: Use inherited theme colors, don't force */
    ...
    section[data-testid="stSidebar"] label {
        color: inherit;
        /* Let Streamlit's theme control colors */
    }
    
    /* Input fields: Don't force text/background colors */
    section[data-testid="stSidebar"] .stTextInput input {
        /* Remove hardcoded colors - let Streamlit handle */
        /* Only customize styling properties */
        border-radius: var(--radius-sm) !important;
    }
    
    /* Table: Let theme control colors naturally */
...
st.set_page_config(page_title="AI PLC-GenStudio", page_icon="app.ico", layout="wide")
st.markdown(
    """
<div class="app-hero">
...
with st.sidebar:
    ...
    with st.expander("🔧 Developer / Connection Settings"):
        backend_url = st.text_input(
            "Backend API Gateway",
            value="https://ai-plc-genstudio.onrender.com/api/v1/generate",
        )
...
def render_timer_block(
    cx: int, y_line: int, comp: dict, variable_lookup: dict
) -> tuple[str, int]:
    c = resolve_element(comp, variable_lookup)
    timer_addr = (
        c.get("address") or comp.get("variable_name") or comp.get("address") or "T0"
    )
    ...
    parts = []
    parts.append(
        f'<line x1="{cx}" y1="{y_line}" x2="{cx + 15}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
    )

    bx = cx + 15
    parts.append(
        f'<rect x="{bx}" y="{box_y}" width="{box_w}" height="{box_h}" fill="#FFFFFF" stroke="#0F172A" stroke-width="2" rx="2" />'
    )
    parts.append(
        f'<text x="{bx + 35}" y="{box_y + 18}" font-size="11" font-family="sans-serif" font-weight="bold" fill="#0F172A">TMR</text>'
    )
    parts.append(
        f'<line x1="{bx}" y1="{box_y + 24}" x2="{bx + box_w}" y2="{box_y + 24}" style="stroke:#0F172A;stroke-width:1" />'
    )

    parts.append(
        f'<text x="{bx + 6}" y="{box_y + 40}" font-size="9" font-family="sans-serif" fill="#1E293B">En</text>'
    )
    parts.append(
        f'<text x="{bx + 6}" y="{box_y + 56}" font-size="9" font-family="sans-serif" fill="#1E293B">S1</text>'
    )
    parts.append(
        f'<text x="{bx + 6}" y="{box_y + 70}" font-size="9" font-family="sans-serif" fill="#1E293B">S2</text>'
    )

    parts.append(
        f'<rect x="{bx + 40}" y="{box_y + 44}" width="48" height="14" fill="#FFFFFF" stroke="#64748B" stroke-width="1" />'
    )
    parts.append(
        f'<text x="{bx + 44}" y="{box_y + 55}" font-size="9" font-family="monospace" fill="#0F172A">{timer_addr}</text>'
    )

    parts.append(
        f'<rect x="{bx + 40}" y="{box_y + 60}" width="48" height="14" fill="#FFFFFF" stroke="#64748B" stroke-width="1" />'
    )
    parts.append(
        f'<text x="{bx + 44}" y="{box_y + 71}" font-size="9" font-family="monospace" fill="#0F172A">{preset_val}</text>'
    )

    exit_x = bx + box_w
    parts.append(
        f'<line x1="{exit_x}" y1="{y_line}" x2="{exit_x + ELEMENT_SPACING}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
    )
...
def render_rung_svg(rung: dict, variable_lookup: dict) -> str:
    ...
    if output_coil:
        ...
        coils_in_rung = [
            ...
            if "COIL" in resolve_element(e, variable_lookup)["type"]
        ]
        if not coils_in_rung and parallel_branches:
            flat_parallel = [
                item
                for sublist in parallel_branches
                for item in (sublist if isinstance(sublist, list) else [sublist])
            ]
            coils_in_rung = [
                e
                for e in flat_parallel
                if "COIL" in resolve_element(e, variable_lookup)["type"]
            ]
        remaining_series = [
            e
            for e in series_elements
            if "COIL" not in resolve_element(e, variable_lookup)["type"]
        ]
    ...
    parts.append(
        f'<svg width="100%" height="{svg_h}" viewBox="0 0 520 {svg_h}" preserveAspectRatio="xMidYMid meet" style="background-color: #FFFFFF;">'
    )

    parts.append(
        f'<line x1="10" y1="0" x2="10" y2="{svg_h}" style="stroke:#0F172A;stroke-width:3" />'
    )
    parts.append(
        f'<line x1="510" y1="0" x2="510" y2="{svg_h}" style="stroke:#0F172A;stroke-width:3" />'
    )

    parts.append(
        f'<line x1="10" y1="{y_line}" x2="40" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
    )
    current_x = 40

    if has_parallel:
        parts.append(
            f'<line x1="{current_x}" y1="{y_line}" x2="{current_x}" y2="{y_line + 70}" style="stroke:#0F172A;stroke-width:2" />'
        )
        parts.append(
            f'<line x1="{current_x}" y1="{y_line}" x2="{current_x + 15}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )
        ...
        t_cx = current_x + 15
        parts.append(
            f'<line x1="{t_cx}" y1="{y_line - 15}" x2="{t_cx}" y2="{y_line + 15}" style="stroke:#0F172A;stroke-width:2" />'
        )
        parts.append(
            f'<line x1="{t_cx + 8}" y1="{y_line - 15}" x2="{t_cx + 8}" y2="{y_line + 15}" style="stroke:#0F172A;stroke-width:2" />'
        )
        parts.append(
            f'<text x="{t_cx - 4}" y="{y_line - 22}" font-size="11" font-family="monospace" font-weight="bold" fill="#1E293B">{top["address"]}</text>'
        )
        parts.append(
            f'<text x="{t_cx - 10}" y="{y_line + 26}" font-size="9" font-family="sans-serif" fill="#64748B">{top["description"]}</text>'
        )
        parts.append(
            f'<line x1="{t_cx + 8}" y1="{y_line}" x2="{current_x + 105}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )

        parts.append(
            f'<line x1="{current_x}" y1="{y_line + 70}" x2="{current_x + 15}" y2="{y_line + 70}" style="stroke:#0F172A;stroke-width:2" />'
        )

        p_branch = parallel_branches[0]
        p_comp = (
            p_branch[0]
            if isinstance(p_branch, list) and len(p_branch) > 0
            else p_branch
        )
        ...
        b_cx = current_x + 15
        parts.append(
            f'<line x1="{b_cx}" y1="{y_line + 55}" x2="{b_cx}" y2="{y_line + 85}" style="stroke:#0F172A;stroke-width:2" />'
        )
        if "NC" in p["type"] or "NOT" in p["type"]:
            parts.append(
                f'<line x1="{b_cx - 3}" y1="{y_line + 82}" x2="{b_cx + 11}" y2="{y_line + 58}" style="stroke:#B91C1C;stroke-width:2" />'
            )
        parts.append(
            f'<line x1="{b_cx + 8}" y1="{y_line + 55}" x2="{b_cx + 8}" y2="{y_line + 85}" style="stroke:#0F172A;stroke-width:2" />'
        )
        parts.append(
            f'<text x="{b_cx - 4}" y="{y_line + 50}" font-size="11" font-family="monospace" font-weight="bold" fill="#1E293B">{p["address"]}</text>'
        )
        parts.append(
            f'<text x="{b_cx - 10}" y="{y_line + 98}" font-size="9" font-family="sans-serif" fill="#64748B">{p["description"]}</text>'
        )
        parts.append(
            f'<line x1="{b_cx + 8}" y1="{y_line + 70}" x2="{current_x + 105}" y2="{y_line + 70}" style="stroke:#0F172A;stroke-width:2" />'
        )
        parts.append(
            f'<line x1="{current_x + 105}" y1="{y_line + 70}" x2="{current_x + 105}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )
    ...
    for comp in remaining_series:
        ...
        if (
            ...
            timer_svg, current_x = render_timer_block(
                current_x, y_line, comp, variable_lookup
            )
            ...
            parts.append(
                f'<line x1="{contact_start_x}" y1="{y_line - 15}" x2="{contact_start_x}" y2="{y_line + 15}" style="stroke:#0F172A;stroke-width:2" />'
            )
            if "NC" in c["type"] or "NOT" in c["type"]:
                parts.append(
                    f'<line x1="{contact_start_x - 3}" y1="{y_line + 12}" x2="{contact_start_x + 11}" y2="{y_line - 12}" style="stroke:#B91C1C;stroke-width:2" />'
                )
            parts.append(
                f'<line x1="{contact_start_x + 8}" y1="{y_line - 15}" x2="{contact_start_x + 8}" y2="{y_line + 15}" style="stroke:#0F172A;stroke-width:2" />'
            )
            parts.append(
                f'<text x="{contact_start_x - 4}" y="{y_line - 22}" font-size="11" font-family="monospace" font-weight="bold" fill="#0F172A">{c["address"]}</text>'
            )
            parts.append(
                f'<text x="{contact_start_x - 10}" y="{y_line + 28}" font-size="9" font-family="sans-serif" fill="#64748B">{c["description"]}</text>'
            )

            addr_text_width = len(str(c.get("address", ""))) * 7
            desc_text_width = len(str(c.get("description", ""))) * 5.5
            occupied_width = max(
                symbol_width + 8, addr_text_width + 12, desc_text_width + 5
            )
            ...
            parts.append(
                f'<line x1="{current_x}" y1="{y_line}" x2="{next_element_start_x}" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
            )
    ...
    if coils_in_rung:
        ...
        if (
            "TIMER" in c_coil_type
            or "TMR" in c_coil_type
            or "TON" in c_coil_type
            or c_coil_addr.startswith("T")
        ):
            has_timer_coil = True

    if coils_in_rung and not has_timer_coil:
        parts.append(
            f'<line x1="{current_x}" y1="{y_line}" x2="435" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )
        c = resolve_element(coils_in_rung[0], variable_lookup)
        parts.append(
            f'<path d="M 435 {y_line - 15} Q 428 {y_line} 435 {y_line + 15}" stroke="#0F172A" stroke-width="2" fill="none"/>'
        )
        parts.append(
            f'<path d="M 450 {y_line - 15} Q 457 {y_line} 450 {y_line + 15}" stroke="#0F172A" stroke-width="2" fill="none"/>'
        )

        if "SET" in c["type"] or "LATCH" in c["type"]:
            parts.append(
                f'<text x="439" y="{y_line + 4}" font-size="10" font-family="sans-serif" font-weight="bold">S</text>'
            )
        elif "RESET" in c["type"] or "UNLATCH" in c["type"]:
            parts.append(
                f'<text x="439" y="{y_line + 4}" font-size="10" font-weight="bold">R</text>'
            )

        parts.append(
            f'<text x="427" y="{y_line - 22}" font-size="11" font-family="monospace" font-weight="bold" fill="#0F172A">{c["address"]}</text>'
        )
        parts.append(
            f'<text x="420" y="{y_line + 28}" font-size="9" font-family="sans-serif" fill="#64748B">{c["description"]}</text>'
        )
        parts.append(
            f'<line x1="450" y1="{y_line}" x2="510" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )
    else:
        parts.append(
            f'<line x1="{current_x}" y1="{y_line}" x2="510" y2="{y_line}" style="stroke:#0F172A;stroke-width:2" />'
        )
...
with col1:
    ...
    st.markdown(
        '<p class="section-title">📝 Functional Specification Prompt</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<p class="section-subtitle">Describe the automation behavior in plain English — the AI compiler handles the rest.</p>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<p class="chip-label">Quick Start Examples</p>', unsafe_allow_html=True
    )
    ...
    with btn_col1:
        generate_btn = st.button(
            "⚙️ Compile & Verify Logic", type="primary", use_container_width=True
        )
    with btn_col2:
            ...
            try:
                with httpx.Client() as client:
                    response = client.post(
                        backend_url, json={"prompt": user_prompt}, timeout=30.0
                    )

                if response.status_code != 200:
                    st.error(
                        f"❌ Automation Pipeline Blocked: {response.json().get('detail', response.text)}"
                    )
                else:
                    ...
                    st.markdown('<div class="section-card">', unsafe_allow_html=True)
                    st.markdown(
                        '<p class="section-title">🛡️ Industrial Safety Audit</p>',
                        unsafe_allow_html=True,
                    )
                    if safety_audit.get("audit_passed", True):
                        ...
                        errors_li = "".join(
                            [
                                f"<li>{err}</li>"
                                for err in safety_audit.get("safety_logs", [])
                            ]
                        )
                        st.markdown(
                    ...
                    st.markdown('<div class="section-card">', unsafe_allow_html=True)
                    st.markdown(
                        '<p class="section-title">📊 Delta Tag Allocation Matrix</p>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if isinstance(v_list, list) and len(v_list) > 0:
                        for v in v_list:
                            if isinstance(v, dict):
                                name_key = (
                                    v.get("name")
                                    or v.get("variable_name")
                                    or v.get("Variable Name")
                                )
                                if name_key:
                                    variable_lookup[str(name_key).strip()] = {
                                        "address": v.get("address")
                                        or v.get("Delta Hardware Address")
                                        or "???",
                    ...
                    if (
                        st_code
                        and st_code.strip()
                        and st_code != "(* Generation Exception *)"
                    ):
                        st.markdown(
                            '<div class="section-card">', unsafe_allow_html=True
                        )
                        st.markdown(
                            '<p class="section-title">💻 Delta ISPSoft Structured Text (ST)</p>',
                            unsafe_allow_html=True,
                        )
                    ...
                    st.markdown('<div class="section-card">', unsafe_allow_html=True)
                    st.markdown(
                        '<p class="section-title">🪜 Graphical Ladder Layout</p>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(
                        "<div class='ladder-rail-container'>", unsafe_allow_html=True
                    )
                    ...
                    if rungs:
                        for rung in rungs:
                            if isinstance(rung, dict):
                                ...
                                st.markdown(
                                    f"<div class='ladder-svg-wrapper'>{svg_markup}</div>",
                                    unsafe_allow_html=True,
                                )
                    else:
                    ...
                    st.markdown('<div class="section-card">', unsafe_allow_html=True)
                    st.markdown(
                        '<p class="section-title">🤖 Live Simulation Diagnostics</p>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if sim_data:
                        ...
                        st.warning(
                            "⚠️ No simulation metadata received from the compiler backend."
                        )
                    st.markdown("</div>", unsafe_allow_html=True)

                    if data.get("developer_notes"):
                        st.info(
                            f"💡 **Delta Deployment Notes:** {data['developer_notes']}"
                        )

            except httpx.ConnectError:
                st.error(
                    "❌ Failed to connect: Backend API is currently offline. Please ensure server is running on local port 8001."
                )
